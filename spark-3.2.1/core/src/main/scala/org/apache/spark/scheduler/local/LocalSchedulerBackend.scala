...
private[spark] class LocalEndpoint(
  ...
  private val curNum = new AtomicInteger(0)
  ...
  def reviveOffers(): Unit = {
    if (curNum.getAndIncrement() == 0) {
      val offers = IndexedSeq(new WorkerOffer(localExecutorId, localExecutorHostname,
        scheduler.CPUS_PER_TASK, Some(rpcEnv.address.hostPort)))
      val start0 = System.nanoTime()
      val got = scheduler.resourceOffers(offers, true).flatten
      val end0 = System.nanoTime()
      val used0 = end0 - start0
      print("resourceOffers first time takes time = " + used0 + " ns\n")
      val got_arr = got.toArray
      freeCores -= scheduler.CPUS_PER_TASK
      executor.runTaskDirectly(executorBackend, got_arr(0))
    }
    // local mode doesn't support extra resources like GPUs right now
    ...
    print("resourceOffers takes time = " + used1 + " ns\n")
    for (task <- got) {
      freeCores -= scheduler.CPUS_PER_TASK
      executor.launchTask(executorBackend, task)
      // // change here to do tests.
      // executor.runTaskDirectly(executorBackend, task)
    }
  }
}
...
private[spark] class LocalSchedulerBackend(
  ...
  override def start(): Unit = {
    ...
    val used = end - start
    print("localSchedulerBackend start takes time = " + used + " ns\n")
  }
