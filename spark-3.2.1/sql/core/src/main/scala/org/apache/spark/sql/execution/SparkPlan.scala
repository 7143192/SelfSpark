...
abstract class SparkPlan extends QueryPlan[SparkPlan] with Logging with Serializable {
  ...
  private def getByteArrayRdd(
    ...
    print("execute in getByteArrayRdd takes time = " + used0 + " ns\n")
    val res = got.mapPartitionsInternal { iter =>
      ...
      print("one time prepare job takes time in " +
        "mapPartitionsInternal takes time = " + used_2 + " ns\n")
      ...
      if (takeFromEnd && n > 0) {
        ...
        // limit is not hit.
        // try to remove the pre-check of this while?
        // while (iter.hasNext) {
        ...
        print("one iter in MapPartitionsInternal takes time = " + used_1 + " ns\n")
      }
      out.writeInt(-1)
      out.flush()
      out.close()
      Iterator((count.toLong, bos.toByteArray))
    }
    res
  }
  ...
  def executeCollect(): Array[InternalRow] = {
    ...
    val end0 = System.nanoTime()
    val used0 = end0 - start0
    print("getByteArrayRdd takes time = " + used0 + " ns\n")
    ...
    print("byteArrayRdd collect takes time = " + used1 + " ns\n")
    val start2 = System.nanoTime()
    collected.foreach { countAndBytes =>
      decodeUnsafeRows(countAndBytes._2).foreach(results.+=)
    }
    val end2 = System.nanoTime()
    val used2 = end2 - start2
    print("decode all rows in executeCollect takes time = " + used2 + " ns\n")
    results.toArray
  }
